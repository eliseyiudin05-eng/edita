...
func TestMetaDoesNotExposeSecrets(t *testing.T) {
	...
	if !strings.Contains(body, `"version":"1.0.52"`) {
...
type fakeEditorDiscussionReader struct {
	result  editordiscussion.Discussion
	created editordiscussion.CreateMessageInput
	err     error
	token   string
	subject string
}

func (reader *fakeEditorDiscussionReader) CreateMessage(_ context.Context, token, subject string, input editordiscussion.CreateMessageInput) (editordiscussion.CreateMessageResponse, error) {
	reader.token = token
	reader.subject = subject
	reader.created = input
	return editordiscussion.CreateMessageResponse{OK: true}, reader.err
}

func (reader *fakeEditorDiscussionReader) Enroll(_ context.Context, token, subject string) (editordiscussion.EnrollResponse, error) {
	reader.token = token
	reader.subject = subject
	return editordiscussion.EnrollResponse{OK: true}, reader.err
}

func (reader *fakeEditorDiscussionReader) GetDiscussion(_ context.Context, token, subject string) (editordiscussion.Discussion, error) {
	reader.token = token
	reader.subject = subject
...
type fakeEditorVerificationReader struct {
	result  editorverification.Verification
	err     error
	token   string
	subject string
}
...
func TestEditorDiscussionRequiresAuthRejectsQueryAndOmitsUserIDs(t *testing.T) {
	reader := &fakeEditorDiscussionReader{result: editordiscussion.Discussion{Messages: []editordiscussion.Message{{
		ID: "223e4567-e89b-12d3-a456-426614174002", Content: "Текст", CreatedAt: "2026-09-14T00:00:00Z", Author: "Монтажёр",
	}}}}
	handler := New(Options{
		Logger:           slog.New(slog.NewTextHandler(io.Discard, nil)),
		Auth:             claimsVerifier{claims: auth.Claims{Subject: "123e4567-e89b-12d3-a456-426614174001", Role: "authenticated"}},
		EditorDiscussion: reader, DependencyTimeout: time.Second,
	})

	unauthorized := httptest.NewRecorder()
	handler.ServeHTTP(unauthorized, httptest.NewRequest(http.MethodGet, "/v1/community/editor-discussion", nil))
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized status = %d", unauthorized.Code)
	}
	invalid := httptest.NewRecorder()
	invalidRequest := httptest.NewRequest(http.MethodGet, "/v1/community/editor-discussion?topic=other", nil)
	invalidRequest.Header.Set("Authorization", "Bearer signed.token.value")
	handler.ServeHTTP(invalid, invalidRequest)
	if invalid.Code != http.StatusBadRequest {
		t.Fatalf("invalid query status = %d", invalid.Code)
	}
	request := httptest.NewRequest(http.MethodGet, "/v1/community/editor-discussion", nil)
	request.Header.Set("Authorization", "Bearer signed.token.value")
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusOK || reader.token != "signed.token.value" || reader.subject != "123e4567-e89b-12d3-a456-426614174001" {
	...
	for _, forbidden := range []string{"123e4567-e89b-12d3-a456-426614174001", "author_id", "user_id", "topic_key", "status", "viewerId"} {
		if strings.Contains(response.Body.String(), forbidden) {
			t.Fatalf("response exposes sensitive value %q: %s", forbidden, response.Body.String())
		}
	}
...
func TestEditorDiscussionCreateRequiresBoundedIdempotentMessage(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/v1/community/editor-discussion", strings.NewReader(`{"id":"223e4567-e89b-12d3-a456-426614174002","content":"Текст"}`))
	request.Header.Set("Authorization", "Bearer signed.token.value")
	request.Header.Set("Content-Type", "application/json")
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
...
func TestEditorDiscussionEnrollmentRequiresPostAndVerifiedSubject(t *testing.T) {
	reader := &fakeEditorDiscussionReader{}
	handler := New(Options{
		Logger:            slog.New(slog.NewTextHandler(io.Discard, nil)),
		Auth:              claimsVerifier{claims: auth.Claims{Subject: "123e4567-e89b-12d3-a456-426614174001", Role: "authenticated"}},
	...
	handler.ServeHTTP(wrongMethod, httptest.NewRequest(http.MethodGet, "/v1/community/editor-discussion/enroll", nil))
	if wrongMethod.Code != http.StatusMethodNotAllowed {
		t.Fatalf("wrong method status=%d", wrongMethod.Code)
	}
	request := httptest.NewRequest(http.MethodPost, "/v1/community/editor-discussion/enroll", nil)
	request.Header.Set("Authorization", "Bearer signed.token.value")
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusOK || reader.token != "signed.token.value" || reader.subject != "123e4567-e89b-12d3-a456-426614174001" || !strings.Contains(response.Body.String(), `"ok":true`) {
		t.Fatalf("unexpected enrollment response: status=%d token=%q subject=%q body=%s", response.Code, reader.token, reader.subject, response.Body.String())
	}
}
...
func TestEditorVerificationRequiresAuthAndOwnerScopedRead(t *testing.T) {
	reader := &fakeEditorVerificationReader{result: editorverification.Verification{
		EmailVerified: true, AgeGroup: "18+", GuardianVerified: false, Level: "basic",
		Request: &editorverification.VerificationRequest{ID: "223e4567-e89b-12d3-a456-426614174002", Status: "pending", CreatedAt: "2026-09-14T00:00:00Z"},
	}}
	handler := New(Options{
		Logger:             slog.New(slog.NewTextHandler(io.Discard, nil)),
		Auth:               claimsVerifier{claims: auth.Claims{Subject: "123e4567-e89b-12d3-a456-426614174001", Role: "authenticated"}},
		EditorVerification: reader,
		DependencyTimeout:  time.Second,
	})

	unauthorized := httptest.NewRecorder()
	handler.ServeHTTP(unauthorized, httptest.NewRequest(http.MethodGet, "/v1/editor/verification", nil))
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized status = %d", unauthorized.Code)
	}

	invalid := httptest.NewRecorder()
	invalidRequest := httptest.NewRequest(http.MethodGet, "/v1/editor/verification?user_id=other", nil)
	invalidRequest.Header.Set("Authorization", "Bearer signed.token.value")
	handler.ServeHTTP(invalid, invalidRequest)
	if invalid.Code != http.StatusBadRequest {
		t.Fatalf("invalid query status = %d", invalid.Code)
	...
	request := httptest.NewRequest(http.MethodGet, "/v1/editor/verification", nil)
	request.Header.Set("Authorization", "Bearer signed.token.value")
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusOK || reader.token != "signed.token.value" || reader.subject != "123e4567-e89b-12d3-a456-426614174001" {
