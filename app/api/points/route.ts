...
export async function GET(req:NextRequest){
  ...
  return NextResponse.json({available:Number(data?.available_points||0),reserved:Number(data?.reserved_points||0),feePercent:12});
}
export async function POST(req:NextRequest){
  ...
  const {data:topup,error}=await service.from("point_topups").insert({user_id:user.id,points,amount_cents:points*100}).select("id").single();
  if(error)return NextResponse.json({error:"Не удалось создать пополнение."},{status:500});
  const payment=await fetch("https://api.yookassa.ru/v3/payments",{method:"POST",headers:{"Content-Type":"application/json","Idempotence-Key":randomUUID(),Authorization:"Basic "+Buffer.from(shop+":"+secret).toString("base64")},body:JSON.stringify({amount:{value:points.toFixed(2),currency:"RUB"},capture:true,confirmation:{type:"redirect",return_url:KIVRONIX_SITE_URL+"/platform#wallet"},description:`Пополнение KIVRONIX на ${points} Points`,metadata:{topup_id:topup.id,user_id:user.id}})});
