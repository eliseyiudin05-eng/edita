...
export async function GET(req:NextRequest){const auth=await access(req);if(!auth)return NextResponse.json({error:"Обсуждение доступно только бизнес-аккаунтам."},{status:403});try{const result=normalizeBusinessDiscussion({messages:await list(auth.service)});if(!result)throw new Error("invalid discussion response");if(businessDiscussionShadowEnabled())after(()=>compareBusinessDiscussionWithGo(auth.accessToken,result));return NextResponse.json(result,{headers:{"Cache-Control":"no-store"}})}catch{return NextResponse.json({error:"Не удалось открыть обсуждение."},{status:503})}}
