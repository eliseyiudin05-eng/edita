...
import {academyLevels,academyStartLevelIndex,assessmentAvailable,curriculum,curriculumStats,finalAssessmentIndex,learningStarts,lessonAccess,nextAcademyStep,normalizeExperienceLevel,optionalCurriculumModules,requiredCurriculum} from "@/lib/curriculum";
...
type Tab="home"|"academy"|"assessment"|"insights"|"practice"|"coach"|"review"|"kivronix-challenges"|"arena"|"portfolio"|"talent"|"partners"|"jobs"|"messages"|"community"|"wallet"|"plans"|"profile"|"business";
type Onboarding={level?:string;software?:string;operatingSystem?:string;goal?:string;ageGroup?:string;accountKind?:string;socialUrl?:string};
type Viewer={
  name:string;
...
const allTabs:[Tab,string][]=[
  ["home","Главная"],["academy","Обучение"],["assessment","Аттестация"],["insights","Лайфхаки"],["practice","Практика"],["coach","Помощник"],["review","Разбор видео"],
...
const lessonXpBySlug=new Map(curriculum.map(lesson=>[lesson.slug,lesson.xp]));
const requiredLessonSlugs=new Set(requiredCurriculum.map(lesson=>lesson.slug));
...
export default function PlatformApp(){
 ...
 const earnedXp=useMemo(()=>done.reduce((sum,slug)=>sum+(lessonXpBySlug.get(slug)||0),0),[done]);
 const requiredDoneCount=useMemo(()=>done.filter(slug=>requiredLessonSlugs.has(slug)).length,[done]);
 const xp=earnedXp;
 ...
 const suggestedLevelIndex=academyStartLevelIndex(experienceLevel);
 const nextStep=nextAcademyStep(done,passedAssessments,experienceLevel);
 const currentLesson=nextStep.kind==="lesson"?nextStep.lesson:null;
 const nextAssessmentModuleIndex=nextStep.kind==="assessment"?nextStep.moduleIndex:-1;
 const routeNeedsAssessment=nextStep.kind==="assessment";
 const tabs=useMemo(()=>{
 ...
 useEffect(()=>{
   ...
   try{
     ...
     const assessmentRaw=localStorage.getItem("kivronix_academy_assessments_v2");
 ...
 return <main className="app">
   ...
   <section className="main">
     ...
     {tab==="home"&&viewer.role!=="business"&&<Page title={viewer.role?"Продолжай, "+viewer.name+".":"Добро пожаловать в KIVRONIX."} sub={viewer.role?"Открой ближайший урок и двигайся по шагам.":"Посмотри платформу. После входа помощник и учебный прогресс сохраняются."}>
       {viewer.role==="editor"?<EditorProgressDashboard xp={xp} completed={requiredDoneCount} total={requiredCurriculum.length} aiScore={viewer.aiScore}/>:null}
       <section className="mission path-mission"><div className="path-mission-copy"><small>ТВОЙ ПУТЬ</small><span className="path-step-label">{routeNeedsAssessment?(nextAssessmentModuleIndex===finalAssessmentIndex?"ВЫПУСКНОЙ ЭТАП":"ПРОВЕРКА УРОВНЯ"):"СЛЕДУЮЩИЙ ШАГ"}</span><h2>{routeNeedsAssessment?(nextAssessmentModuleIndex===finalAssessmentIndex?"Выпускной экзамен":`Аттестация «${academyLevels[nextAssessmentModuleIndex]?.name}»`):currentLesson?currentLesson.title:"Маршрут завершён"}</h2><p>{routeNeedsAssessment?"Подтверди навыки, чтобы продолжить маршрут. Следующий уровень откроется после успешной аттестации.":currentLesson?currentLesson.summary:"Все уроки и проверки пройдены. Возвращайся к материалам и усиливай портфолио."}</p></div><div className="path-mission-progress"><strong>{earnedXp}</strong><span>опыта</span><small>{requiredDoneCount} из {requiredCurriculum.length} уроков</small></div>{routeNeedsAssessment?<Link className="btn btn-lime" href={`/academy/assessment/${nextAssessmentModuleIndex}`}>{nextAssessmentModuleIndex===finalAssessmentIndex?"Перейти к экзамену":"Пройти аттестацию"} →</Link>:currentLesson?<Link className="btn btn-lime" href={`/academy/${currentLesson.slug}`}>Продолжить обучение →</Link>:<button className="btn btn-lime" onClick={()=>goTab("academy")}>Открыть обучение →</button>}</section>
       <div className="grid">
     ...
     {tab==="academy"&&<Page title="Обучение" sub="Три уровня от первого проекта до коммерческой работы. Один урок открывается после подтверждения предыдущего.">
       <div className="academy-start-card"><div><div className="eyebrow">{currentLesson?"СЛЕДУЮЩИЙ УРОК":routeNeedsAssessment?"ПОРА ПРОЙТИ АТТЕСТАЦИЮ":"МАРШРУТ ПРОЙДЕН"} · {suggestedStart.label.toUpperCase()}</div><h2>{currentLesson?.title||(routeNeedsAssessment?(nextAssessmentModuleIndex===finalAssessmentIndex?"Выпускной экзамен":`Аттестация «${academyLevels[nextAssessmentModuleIndex]?.name}»`):"Все обязательные уроки завершены")}</h2><p>{routeNeedsAssessment?"Уроки уровня завершены. ИИ проверит работы по рубрике, а звук, склейки и права ты подтвердишь отдельным чек‑листом.":currentLesson?"Выполни действие, пройди мини‑тест и опиши результат. После этого откроется следующий урок.":"Возвращайся к материалам и усиливай портфолио."}</p></div>{currentLesson?<Link className="btn btn-lime" href={"/academy/"+currentLesson.slug}>Открыть урок →</Link>:routeNeedsAssessment?<Link className="btn btn-lime" href={`/academy/assessment/${nextAssessmentModuleIndex}`}>{nextAssessmentModuleIndex===finalAssessmentIndex?"Перейти к экзамену":"Перейти к аттестации"} →</Link>:null}</div>
       <div className="academy-overview">
         <Stat n={String(curriculumStats.lessons)} t="обязательных уроков"/><Stat n={String(curriculumStats.theory)} t="уроков понимания"/><Stat n={String(curriculumStats.assignments)} t="практик"/><Stat n={String(requiredDoneCount)} t="пройдено"/>
       </div>
       <div className="academy-route-note"><b>Как здесь учиться?</b><span>Объяснение → показ → действие вместе → самостоятельная работа → мини‑тест → исправление. Следующий урок открывается после проверяемого результата, следующий уровень — после аттестации.</span></div>
       <div className="academy-modules">{academyLevels.map((level,levelIndex)=><section className="academy-module academy-level" key={level.id}>
         <header><div><div className="eyebrow">УРОВЕНЬ {levelIndex+1} · {level.name}</div><h2>{level.title}</h2><p>{level.outcome}</p><small><b>Проект уровня:</b> {level.project}</small></div><span>{level.lessons.filter(item=>done.includes(item.slug)).length} / {level.lessons.length}</span></header>
         <div className="academy-lesson-grid">{level.lessons.map(lesson=>{
           const lessonIndex=requiredCurriculum.findIndex(item=>item.slug===lesson.slug);
           ...
           return <article className={"academy-lesson-card "+(done.includes(lesson.slug)?"done ":"")+(unlocked?"":"locked")} key={lesson.slug}>
             <div className="academy-lesson-top"><span className="num">{done.includes(lesson.slug)?"✓":unlocked?lessonIndex+1:"—"}</span><div><small>{lesson.level} · {lesson.minutes} мин</small><b>{unlocked?lesson.software:access.reason==="assessment"?"После аттестации":"После предыдущего урока"}</b></div></div>
             <h3>{lesson.title}</h3><p>{lesson.summary}</p>
             <div className="academy-tags"><span>{lesson.track}</span><span>{lesson.theoryOnly?"Понимание + мини‑тест":"Практика + доказательство"}</span><span>4 редактора</span><span>Помощник в уроке</span></div>
             <div className="lesson-actions">{unlocked?<Link className="btn btn-dark" href={"/academy/"+lesson.slug}>{done.includes(lesson.slug)?"Открыть урок снова":"Открыть урок"}</Link>:<button className="btn btn-ghost" disabled>{access.reason==="assessment"?"Сначала аттестация":"Сначала предыдущий урок"}</button>}<span className="lesson-xp">+{lesson.xp} опыта</span></div>
           </article>
         })}</div>
         {passedAssessments.includes(levelIndex)?<div className="academy-assessment-preview passed"><b>✓ Уровень {level.name} подтверждён</b><span>Результат сохранён. Следующий уровень открыт.</span><Link className="btn btn-ghost" href={`/academy/assessment/${levelIndex}`}>Посмотреть результат</Link></div>:assessmentAvailable(levelIndex,done,passedAssessments,experienceLevel)?<div className="academy-assessment-preview ready"><b>Пора подтвердить уровень {level.name}</b><span>Проект, рубрика на 100 баллов, мини‑тест и ручная проверка того, что ИИ не видит.</span><Link className="btn btn-dark" href={`/academy/assessment/${levelIndex}`}>Открыть аттестацию →</Link></div>:<div className="academy-assessment-preview"><b>Аттестация уровня {level.name}</b><span>{levelIndex<suggestedLevelIndex?"Уровень находится до твоей рекомендованной стартовой точки.":"Заверши уроки по порядку. После успешной проверки откроется следующий уровень."}</span></div>}
       </section>)}</div>
       <section className="academy-optional-library"><header><div><div className="eyebrow">ПО ЖЕЛАНИЮ · НЕ ВЛИЯЕТ НА ПРОГРЕСС</div><h2>Библиотека редакторов</h2><p>Здесь можно посмотреть отдельные вводные по программам. Основные уроки уже адаптируются под выбранный CapCut Desktop, Premiere Pro, DaVinci Resolve или Final Cut Pro.</p></div><span>{curriculumStats.optional} материалов</span></header><div className="academy-lesson-grid">{optionalCurriculumModules.flatMap(group=>group.lessons).map(lesson=><article className={"academy-lesson-card "+(done.includes(lesson.slug)?"done":"")} key={lesson.slug}><div className="academy-lesson-top"><span className="num">{done.includes(lesson.slug)?"✓":"+"}</span><div><small>{lesson.minutes} мин · справочник</small><b>{lesson.software}</b></div></div><h3>{lesson.title}</h3><p>{lesson.summary}</p><div className="lesson-actions"><Link className="btn btn-ghost" href={"/academy/"+lesson.slug}>Открыть материал</Link></div></article>)}</div></section>
       <section className={"graduation-exam-card "+(passedAssessments.includes(finalAssessmentIndex)?"passed":assessmentAvailable(finalAssessmentIndex,done,passedAssessments,experienceLevel)?"ready":"locked")}><div><div className="eyebrow">ВЫПУСК · ПОСЛЕ УРОВНЯ PRO</div><h2>Отдельный выпускной экзамен</h2><p>{passedAssessments.includes(finalAssessmentIndex)?"Экзамен пройден. Полный маршрут KIVRONIX завершён.":assessmentAvailable(finalAssessmentIndex,done,passedAssessments,experienceLevel)?"Представь три лучшие разные работы как готовое портфолио.":"Экзамен откроется после обязательных уроков и аттестаций твоего маршрута."}</p></div>{passedAssessments.includes(finalAssessmentIndex)?<Link className="btn btn-ghost" href={`/academy/assessment/${finalAssessmentIndex}`}>Посмотреть результат</Link>:assessmentAvailable(finalAssessmentIndex,done,passedAssessments,experienceLevel)?<Link className="btn btn-lime" href={`/academy/assessment/${finalAssessmentIndex}`}>Перейти к экзамену →</Link>:<span className="exam-lock">Закрыто</span>}</section>
     </Page>}

     {tab==="assessment"&&<Page title="Аттестация" sub="Одна проверка после каждого уровня и отдельный выпускной экзамен после PRO.">
       <section className="assessment-menu-hero"><div><div className="eyebrow">ТЕКУЩАЯ ПРОВЕРКА</div><h2>{routeNeedsAssessment?(nextAssessmentModuleIndex===finalAssessmentIndex?"Выпускной экзамен":`Аттестация «${academyLevels[nextAssessmentModuleIndex]?.name}»`):"Сначала заверши текущий уровень"}</h2><p>{routeNeedsAssessment?"Загрузи работы, ответь на три вопроса, пройди ручной чек‑лист и получи разбор по критериям.":"Когда завершишь последовательность уроков, здесь появится доступная аттестация."}</p></div>{routeNeedsAssessment?<Link className="btn btn-lime" href={`/academy/assessment/${nextAssessmentModuleIndex}`}>Открыть проверку →</Link>:<button className="btn btn-dark" onClick={()=>goTab("academy")}>Перейти к урокам</button>}</section>
       <div className="assessment-stage-list">{academyLevels.map((level,levelIndex)=><article className={passedAssessments.includes(levelIndex)?"passed":assessmentAvailable(levelIndex,done,passedAssessments,experienceLevel)?"ready":"locked"} key={level.id}><span>{passedAssessments.includes(levelIndex)?"✓":levelIndex+1}</span><div><b>{level.name} · {level.title}</b><small>{passedAssessments.includes(levelIndex)?"Пройдена":assessmentAvailable(levelIndex,done,passedAssessments,experienceLevel)?"Доступна":"Заверши уровень и предыдущие проверки"}</small></div>{assessmentAvailable(levelIndex,done,passedAssessments,experienceLevel)||passedAssessments.includes(levelIndex)?<Link href={`/academy/assessment/${levelIndex}`}>Открыть →</Link>:null}</article>)}<article className={passedAssessments.includes(finalAssessmentIndex)?"passed":assessmentAvailable(finalAssessmentIndex,done,passedAssessments,experienceLevel)?"ready":"locked"}><span>{passedAssessments.includes(finalAssessmentIndex)?"✓":"★"}</span><div><b>Выпускной экзамен</b><small>{passedAssessments.includes(finalAssessmentIndex)?"Маршрут завершён":assessmentAvailable(finalAssessmentIndex,done,passedAssessments,experienceLevel)?"Доступен":"После уровня PRO"}</small></div>{assessmentAvailable(finalAssessmentIndex,done,passedAssessments,experienceLevel)||passedAssessments.includes(finalAssessmentIndex)?<Link href={`/academy/assessment/${finalAssessmentIndex}`}>Открыть →</Link>:null}</article></div>
     </Page>}
     ...
     {tab==="profile"&&viewer.role!=="business"&&<Page title="Профиль" sub="Навыки, персональный маршрут и публичная карьерная карточка.">
       <div className="profile-grid">
         ...
         <Card title="О себе"><div className="editor-profile-description"><b>Монтажёр Reels для экспертов и авторов</b><p>Субтитры, динамичный монтаж, музыка и оформление</p><p>Работаю в Adobe Premiere Pro</p><p>Портфолио — по ссылке</p><p>Заказ и вопросы — в Direct</p></div></Card>
