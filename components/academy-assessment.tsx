...
export default function AcademyAssessment({moduleIndex,moduleName,final=false,passed,onPassed}:{moduleIndex:number;moduleName:string;final?:boolean;passed:boolean;onPassed:()=>void}){
  ...
  async function review(){
    ...
    try{
      ...
      if(!session)throw new Error("Войди в аккаунт, чтобы AI оценил работы.");
      ...
      if(workAverage>=threshold&&quizScore>=80){onPassed();setMessage(final?"Экзамен сдан. Путь академии завершён — ты готов к сложным проектам.":"Ступень пройдена. Следующий уровень открыт.")}
  ...
  return <section className={"academy-assessment "+(passed?"passed":"")}>
    <header><div><div className="eyebrow">{final?"ФИНАЛЬНЫЙ ЭКЗАМЕН":"ПЕРЕХОД НА СЛЕДУЮЩУЮ СТУПЕНЬ"}</div><h3>{final?"5 работ + общий тест":"3 работы + мини‑тест"}</h3><p>{passed?"Аттестация пройдена. Результат сохранён в этом браузере.":`AI оценит каждую работу по шкале 1–100. Проходной балл — ${threshold}, тест — от 80.`}</p></div><div className="assessment-threshold"><strong>{threshold}</strong><span>минимум</span></div></header>
